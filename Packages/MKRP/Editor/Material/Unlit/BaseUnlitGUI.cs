...
namespace KuanMiEditor.Rendering.MKRP
{
    ...
    {
        public static void SetupBaseUnlitKeywords(this UnityEngine.Material material)
        {
            // First thing, be sure to have an up to date RenderQueue
            material.ResetMaterialCustomRenderQueue();

            bool alphaTestEnable = material.HasProperty(kAlphaCutoffEnabled) &&
                                   material.GetFloat(kAlphaCutoffEnabled) > 0.0f;
            ...
            float alphaToMaskEnabled = material.HasProperty("_AlphaToMaskInspectorValue") &&
                                       material.GetFloat("_AlphaToMaskInspectorValue") > 0.0
                ? 1
                : 0;
            material.SetFloat(kAlphaToMask, alphaTestEnable ? alphaToMaskEnabled : 0);

            bool alphaToMaskEnable = alphaTestEnable && material.HasProperty(kAlphaToMask) &&
                                     material.GetFloat(kAlphaToMask) > 0.0f;
            ...
            bool transparentWritesMotionVec = (surfaceType == SurfaceType.Transparent) &&
                                              material.HasProperty(kTransparentWritingMotionVec) &&
                                              material.GetInt(kTransparentWritingMotionVec) > 0;
            CoreUtils.SetKeyword(material, "_TRANSPARENT_WRITES_MOTION_VEC", transparentWritesMotionVec);

            MKRenderQueue.RenderQueueType renderQueueType =
                MKRenderQueue.GetTypeByRenderQueueValue(material.renderQueue);
            bool needOffScreenBlendFactor =
                renderQueueType == MKRenderQueue.RenderQueueType.AfterPostprocessTransparent ||
                renderQueueType == MKRenderQueue.RenderQueueType.LowTransparent;
            ...
            if (material.HasProperty(kZTestDepthEqualForOpaque))
            {
                if (surfaceType == SurfaceType.Opaque)
                {
                    ...
                    if (MKRenderQueue.k_RenderQueue_AfterPostProcessOpaque.Contains(material.renderQueue))
                        material.SetInt(kZTestDepthEqualForOpaque,
                    ...
                    switch (blendMode)
                    {
                        ...
                        case BlendMode.Alpha:
                            ...
                            if (needOffScreenBlendFactor)
                            {
                                ...
                                material.SetInt("_AlphaDstBlend",
            ...
            bool fogEnabled = material.HasProperty(kEnableFogOnTransparent) &&
                              material.GetFloat(kEnableFogOnTransparent) > 0.0f &&
            ...
            CullMode doubleSidedOffMode = (surfaceType == SurfaceType.Transparent)
                ? material.GetTransparentCullMode()
                : material.GetOpaqueCullMode();

            bool isBackFaceEnable = material.HasProperty(kTransparentBackfaceEnable) &&
                                    material.GetFloat(kTransparentBackfaceEnable) > 0.0f &&
                                    surfaceType == SurfaceType.Transparent;
            bool doubleSidedEnable =
                material.HasProperty(kDoubleSidedEnable) && material.GetFloat(kDoubleSidedEnable) > 0.0f;

            // Disable culling if double sided
            material.SetInt("_CullMode",
                doubleSidedEnable ? (int) UnityEngine.Rendering.CullMode.Off : (int) doubleSidedOffMode);
            ...
            {
                material.SetInt("_CullModeForward",
                    (int) (doubleSidedEnable ? UnityEngine.Rendering.CullMode.Off : doubleSidedOffMode));
            }
            ...
            {
                material.SetColor(kEmissionColor,
                    Color.white); // kEmissionColor must always be white to allow our own material to control the GI (this allow to fallback from builtin unity to our system).
        ...
        static public void SetupBaseUnlitPass(this Material material)
        {
            ...
            if (false)
            {
                ...
                // all the time here.
                //material.SetShaderPassEnabled(MKShaderPassNames.s_DistortionVectorsStr, true);
            }
            else if (material.HasProperty(kDistortionEnable))
            {
                ...
                material.SetShaderPassEnabled(MKShaderPassNames.s_ForwardStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_DepthOnlyStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_DepthForwardOnlyStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_ForwardOnlyStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_GBufferStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_GBufferWithPrepassStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_DistortionVectorsStr, distortionEnable); // note: use distortionEnable
                material.SetShaderPassEnabled(MKShaderPassNames.s_TransparentDepthPrepassStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_TransparentBackfaceStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_TransparentDepthPostpassStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_RayTracingPrepassStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_MetaStr, enablePass);
                material.SetShaderPassEnabled(MKShaderPassNames.s_ShadowCasterStr, enablePass);
            }
            ...
            if (material.HasProperty(kTransparentDepthPrepassEnable))
            {
                ...
                material.SetShaderPassEnabled(MKShaderPassNames.s_TransparentDepthPrepassStr, depthWriteEnable || ssrTransparent);
            }
            ...
            if (material.HasProperty(kTransparentDepthPostpassEnable))
            {
                ...
                material.SetShaderPassEnabled(MKShaderPassNames.s_TransparentDepthPostpassStr, depthWriteEnable);
            }
            ...
            if (material.HasProperty(kTransparentBackfaceEnable))
            {
                ...
                material.SetShaderPassEnabled(MKShaderPassNames.s_TransparentBackfaceStr, backFaceEnable);
            }

            if (material.HasProperty(kRayTracing))
            {
                bool rayTracingEnable = (material.GetFloat(kRayTracing) > 0.0f);
                material.SetShaderPassEnabled(MKShaderPassNames.s_RayTracingPrepassStr, rayTracingEnable);
            }
            ...
            if (true)
            {
                ...
                material.SetShaderPassEnabled(MKShaderPassNames.s_MotionVectorsStr, addPrecomputedVelocity);
             }
        }
        ...
        public static void SetupMainTexForAlphaTestGI(this Material material, string colorMapPropertyName,
